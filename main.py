# Virtual - System Control using Gestures.
...
while True:
    ...
    if len(landmarklist)!=0:
        ...
        if fingers[1] == 1 and fingers[2] == 1:
            ...
            if length<20:
                ...
                autopy.mouse.click()
        # Checking if index finger is down and middle finger is up to right click
        ...
        # Checking if thumb finger is up to scroll down
        if fingers[0] == 1 and fingers[4] == 0:
            length, img, lineInfo = detector.findDistance(8, 12, img)
            cv2.circle(img, (lineInfo[4], lineInfo[5]), 15, (0, 255, 255), cv2.FILLED)
            mouse.wheel(-1)

        # Checking if pinky finger is up to scroll up
    ...
    # Listening Virtual Mouse via camera
    cv2.imshow("Gesture Mouse - BATCH 16 - CSE - BIT", img)
    cv2.waitKey(1)